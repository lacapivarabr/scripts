--=== AUTO BUY VIP VERSÃO 04 (COM ABA BACKPACK) ===--
-- Abas: Compras, Extras (Anti-AFK + Pulo Alto) e Backpack (contador de itens)
...
titulo.Size = UDim2.new(0, 100, 1, 0)  -- Reduzido para 100 para dar espaço às abas
...
-- Botões de aba (agora 3)
local abaCompras = Instance.new("TextButton")
abaCompras.Size = UDim2.new(0, 50, 0, 30)
abaCompras.Position = UDim2.new(0, 115, 0, 5)
...
abaExtras.Size = UDim2.new(0, 50, 0, 30)
abaExtras.Position = UDim2.new(0, 170, 0, 5)
abaExtras.BackgroundColor3 = Color3.fromRGB(60, 60, 70)
abaExtras.Text = "Extras"
abaExtras.TextColor3 = Color3.fromRGB(200, 200, 200)
abaExtras.TextSize = 11
abaExtras.Font = Enum.Font.GothamBold
abaExtras.BorderSizePixel = 0
abaExtras.Parent = tituloFrame

local abaBackpack = Instance.new("TextButton")
abaBackpack.Size = UDim2.new(0, 50, 0, 30)
abaBackpack.Position = UDim2.new(0, 225, 0, 5)
abaBackpack.BackgroundColor3 = Color3.fromRGB(60, 60, 70)
abaBackpack.Text = "Backpack"
abaBackpack.TextColor3 = Color3.fromRGB(200, 200, 200)
abaBackpack.TextSize = 11
abaBackpack.Font = Enum.Font.GothamBold
abaBackpack.BorderSizePixel = 0
abaBackpack.Parent = tituloFrame
...
-- ===== PÁGINA BACKPACK (CONTADOR DE ITENS) ===== --
local paginaBackpack = Instance.new("Frame")
paginaBackpack.Name = "PaginaBackpack"
paginaBackpack.Size = UDim2.new(1, 0, 1, -40)
paginaBackpack.Position = UDim2.new(0, 0, 0, 40)
paginaBackpack.BackgroundTransparency = 1
paginaBackpack.Visible = false
paginaBackpack.Parent = frame

-- Título
local backpackTitulo = Instance.new("TextLabel")
backpackTitulo.Size = UDim2.new(1, -20, 0, 30)
backpackTitulo.Position = UDim2.new(0, 10, 0, 10)
backpackTitulo.BackgroundTransparency = 1
backpackTitulo.Text = "🎒 BACKPACK"
backpackTitulo.TextColor3 = Color3.fromRGB(255, 255, 255)
backpackTitulo.TextSize = 16
backpackTitulo.Font = Enum.Font.GothamBold
backpackTitulo.Parent = paginaBackpack

-- Contador total de itens
local totalLabel = Instance.new("TextLabel")
totalLabel.Size = UDim2.new(1, -20, 0, 60)
totalLabel.Position = UDim2.new(0, 10, 0, 50)
totalLabel.BackgroundColor3 = Color3.fromRGB(40, 40, 50)
totalLabel.Text = "0"
totalLabel.TextColor3 = Color3.fromRGB(100, 255, 100)
totalLabel.TextSize = 48
totalLabel.Font = Enum.Font.GothamBold
totalLabel.Parent = paginaBackpack

local totalBorda = Instance.new("UICorner")
totalBorda.CornerRadius = UDim.new(0, 8)
totalBorda.Parent = totalLabel

-- Função para atualizar contagem
local function atualizarContagemBackpack()
    local backpack = player:FindFirstChild("Backpack")
    if backpack then
        local count = #backpack:GetChildren()
        totalLabel.Text = tostring(count)
    else
        totalLabel.Text = "0"
    end
end

-- Atualiza quando a aba for aberta e a cada 5 segundos
local function quandoAbrirBackpack()
    atualizarContagemBackpack()
    -- Opcional: conectar a eventos de mudança no backpack? (simples, só atualizar periodicamente)
end
...
local function loopCompra()
    ...
    while ATIVO do
        ...
        -- Atualiza timer do restock (caminho adaptável)
        local restockObj = player.PlayerGui:FindFirstChild("BabyCatsGui") and
        ...
        if restockObj and restockObj:IsA("TextLabel") then
...
local function selecionarAba(aba)
    ...
    if aba == "Compras" then
        abaCompras.BackgroundColor3 = Color3.fromRGB(0, 120, 200)
        abaCompras.TextColor3 = Color3.fromRGB(255, 255, 255)
    elseif aba == "Extras" then
        abaExtras.BackgroundColor3 = Color3.fromRGB(0, 120, 200)
        abaExtras.TextColor3 = Color3.fromRGB(255, 255, 255)
    elseif aba == "Backpack" then
        abaBackpack.BackgroundColor3 = Color3.fromRGB(0, 120, 200)
        abaBackpack.TextColor3 = Color3.fromRGB(255, 255, 255)
        quandoAbrirBackpack() -- Atualiza contagem ao abrir
    end
    
    paginaCompras.Visible = (aba == "Compras")
    paginaExtras.Visible = (aba == "Extras")
    paginaBackpack.Visible = (aba == "Backpack")
end
...
abaBackpack.MouseButton1Click:Connect(function()
    selecionarAba("Backpack")
end)
...
-- Mensagens iniciais
addLog("✅ VIP Auto Buy v04 (com Backpack)", "info")
addLog("🔽 Abas: Compras, Extras, Backpack", "info")
if itens.OP.objeto then addLog("⭐ STAR: Disponível", "sucesso") end
if itens.SECRET.objeto then addLog("🥛🍪 Milk: Disponível", "sucesso") end

print("=== AUTO BUY VIP VERSÃO 04 (COM BACKPACK) ===")
print("✅ Nova aba Backpack mostra total de itens")
